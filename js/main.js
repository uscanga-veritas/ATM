...
let cuentas = [
    { nombre: 'Joe', saldo: 0, retiro: 0, deposito: 0, password: 'rappi123'},
    { nombre: 'Memo', saldo: 0, retiro: 0, deposito: 0, password: 'panda123'},
    { nombre: 'Victor', saldo: 0, retiro: 0, deposito: 0, password: 'iecc123'}
];
...
if (username === '' && password === ''){
    alert('Falta escribir ambos campos')
    return;
}
else if (username === '') {
...
for (let i = 0; i < cuentas.length; i++) {
    if (cuentas[i].nombre === username && cuentas[i].password === password) {
        valid = true;
            ...
            document.querySelector('.text-p-m').textContent = `Bienvenido ${username}`;
    ...
    if (!valid) {
        alert('ERROR, el usuario y/o la contraseña son inválidos');
    }
}
...
function ok() {
    ...
    if (action === 'r1') {
        ...
        if(dineroInput.value === ''){
            alert('ERROR, favor de escribir un monto válido');
        }else{
            ...
            if(userAccount.saldo + userAccount.deposito <991){
                ...
                alert('UPS, excede del límite actual ($990 pesos)');
                dineroInput.value = '';
        ...
        if(dineroInput.value === ''){
            ...
            if(userAccount.saldo - userAccount.retiro >9){
                ...
                alert('UPS, excede del límite mínimo actual ($10 pesos)');
                dineroInput.value = '';
...
function updateMenu() {
    ...
    if (userAccount.saldo >= 0 && userAccount.saldo <= 9) {
      ...
      if (action === 'r1') {
        ...
        dineroInput.value = ' Balance: $' + userAccount.saldo;
      ...
      if (action === 'r1') {
        ...
        if (userAccount.saldo === 990) {
          alert('Has llegado al límite máximo de su cuenta ($990 pesos)');
        } else if (userAccount.saldo === 10) {
          alert('Has llegado al límite mínimo de su cuenta ($10 pesos)');
        }
      } else if (action === 'r2' || action === 'r3') {
        buttons.forEach(button => button.disabled = false);
        document.getElementById('cancel').style.backgroundColor = '';
